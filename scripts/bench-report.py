...
def _(alt, df):
    build_chart = (
        ...
        .encode(
            ...
            tooltip=["label", "build_ms", "bench_unavailable", "subject"],
        )
    ...
    if "shader_validation_us" not in df.columns:
        ...
        shader_chart = (
            ...
            .encode(
                ...
                tooltip=["label", "shader_validation_us", "subject"],
            )
...
def _(alt, df, mo, pl):
    ...
    if not _have:
        ...
        _df_g = df.filter(~pl.col("bench_unavailable") & ~pl.col("gpu_skipped")).select(
            ["label", "sphere_128_64_us", "scene_update_us", "subject"]
        )
        if _df_g.is_empty():
            gpu_chart = None
            _out = mo.callout(
            ...
            gpu_chart = (
                ...
                .encode(
                    ...
                    tooltip=["label", "metric", "us", "subject"],
                )
...
def _(Path, build_chart, df, gpu_chart, pl, shader_chart):
    ...
    # Markdown report
    _want = ["label", "build_ms", "shader_validation_us", "sphere_128_64_us", "scene_update_us", "subject"]
    _cols = [c for c in _want if c in df.columns]
    ...
    Path("perf/bench-report.md").write_text(md_report_text)

    export_status = f"Wrote {len(_written)} SVG(s) → `perf/plots/` and `perf/bench-report.md`"
    export_status
    return (md_report_text,)


@app.cell(hide_code=True)
def _(md_report_text, mo):
    mo.md(md_report_text)
    return
