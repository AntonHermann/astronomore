...
| Torus-Shader-Zitat in Vergleichs-Doku gegen Buch prüfen | `docs/opengl-wgpu-mapping.html`, Stufe 06, linke Spalte | Inhaltsreview vom 10.07. fand drei fachliche Auffälligkeiten im zitierten Buch-Code (`gl_ModelViewProjectionMatrix` im Licht-Vektor statt ModelView; `reflect(l, n)` ohne Negation von `l`; `max` außerhalb statt innerhalb von `pow`) — kann Übertragungsfehler oder Original-Buch-Macke sein | Anton: gegen `torusambientdiffusespecularfinal.vs` abgleichen; falls Buch-Macke, ggf. als Pointe für den Vortrag nutzen |
