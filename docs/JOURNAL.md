...
| Faden | Abschluss |
|---|---|
| Render-Draw-Ablauf-Übersicht (`docs/draw-flow.html`): drei Phasen des Frame-Lebenszyklus (Datenstrukturen anlegen in `State::new`, updaten in `State::update`/`Scene::update`, zeichnen in `State::render`) mit echten `datei:zeile`-Ankern und Bind-Group-Tabelle. Zusätzlich ein Matrix-Register (Herkunft/Haltung/Verwendung jeder Matrix), das hervorhebt, dass nur zwei Matrizen tatsächlich in GPU-Buffern liegen — Model (`world·spin·scale`, Group 2, pro Körper) und `view_proj` (`projection·view`, Group 1). Als Claude-Artefakt publiziert und ins Repo übernommen | 10.07. — direkt auf main |
| Pages-404 des Begleitdokument-Links behoben: die deployte `opengl-wgpu-mapping.html` verlinkte relativ auf `./opengl-wgpu-analysis.md`, das der Pages-Workflow nicht publiziert (nur die HTML) → 404. Link (und README-Erwähnung) auf die GitHub-Blob-URL umgebogen, Rücklink in `analysis.md` auf die Pages-URL der `mapping.html` gesetzt. **Bewusst akzeptierter Tradeoff:** Repo ist privat → der GitHub-Blob-Link funktioniert nur für eingeloggte Reviewer mit Repo-Zugriff, für anonyme Pages-Besucher bleibt das Begleitdokument unerreichbar. Alternativen (md→HTML auf Pages rendern / roh kopieren / Repo öffentlich) verworfen. Wenn das Repo später öffentlich wird, funktionieren die Links automatisch | 10.07. — gemergt in main (`723c6c6`), Branch gelöscht |
